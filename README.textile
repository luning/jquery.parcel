...
It represents a partial of part. You can treat a virtual field as a normal Field, that means you can call @state()@, @isDirty()@, @resetState()@, and do event operations like @bindState()@, @stateChange()@ on it.
...
Field in part is the core concept in Coconut, it can be an extended *jQuery* object, an extended *Array* contains other fields or another *Part*. *Virtual Field* is not necessarily being stored internally in its parent part, but can be treated as a normal field. All these forms the concept Field and all field operations and events are availiable to them.
