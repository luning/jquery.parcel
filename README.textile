...
Then you create a *part* like this:

<pre>
  var person = $("#person").part();
</pre>

This builds a *part* representing _person_ with all *fields* set up automatically from their *name* attribute of the input field.

The fields are actually jQuery wrappers of each input in the *part*.
...
This is just like _person.firstname.val("John")_ in jQuery, but with the appropriate events are fired as if a real user typed the value into the field.
...
You can tell if the state has changed with _isDirty()_:
