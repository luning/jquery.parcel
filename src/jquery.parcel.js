    ...
    state: function(s, option){
      ...
      if (s === undefined){
        ...
        try {
          ...
          if (option.check && !$.stateContain(this.state(), s)){
            throw "ParcelError: failed to set state with [" + s + "]";
          }
        } finally {
          if (option.sync){
      ...
      if (this._nameConstraint){
        $.each(state, function(i, curState){
          if (i < fields.length){
            ...
            // throw if ensure existent is specified in option and failed to do this setting eventually
              ...
              throw "ParcelError: no UI element found while setting state for the [" + (i + 1) + "th] " + this._nameConstraint + " with [" + state[i] + "]";
            }
          }
        } .bind(this));
      } else {
        ...
        if (option.exist && !$.stateEmpty(state)){
          for (var key in state){
            throw "ParcelError: no UI element found while setting state for [" + key + "] with [" + state[key] + "]";
          }
        }
      }
      return this;
